...
网页还可以勾选需要导出的镜像，再点击「生成镜像包」触发 `export-images.yaml`。GitHub Action 会从阿里云镜像仓库拉取选中的镜像，执行 `docker save` 生成 `docker-images.tar.gz`，并上传为 GitHub Actions artifact。生成完成后，网页会显示可直接用浏览器下载的链接。
