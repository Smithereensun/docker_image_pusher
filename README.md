...
Cloudflare Pages 的 `public/` 目录除了镜像管理，还提供一组纯前端本地工具。新增入口如下：

- `图片去水印`：上传图片后，在预览区拖拽框选一个或多个矩形区域，可选择 `邻近填充`、`白底覆盖`、`局部模糊` 三种方式处理，再直接下载。
- `PDF 去水印`：上传 PDF 后，在页面预览中框选区域；可选择 `白底覆盖（保留 PDF 结构）`，或选择 `邻近填充 / 局部模糊（逐页重建 PDF）`。

使用说明与限制：

- 所有图片、PDF 内容都只在当前浏览器本地处理，不会上传到服务端。
- `白底覆盖` 更适合白底文档、扫描件或截图类水印。
- `邻近填充` 更适合背景较简单、边缘颜色连续的角标或小面积水印。
- `PDF` 的逐页重建模式会把页面栅格化后重新生成 PDF，适合图片水印，但文件体积可能变大，文字将不再保留原始矢量层。
- 动图、复杂纹理背景、跨越正文的大面积半透明水印，建议先尝试局部模糊或改用更专业的图像修复工具。
